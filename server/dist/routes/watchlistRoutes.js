...
router.post("/", authMiddleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (!user.watchlist.some((id) => id.toString() === existingMovie._id.toString())) {
            user.watchlist.push(existingMovie._id);
            await user.save();
        }
...
router.delete("/:movieId", authMiddleware_1.authenticate, async (req, res) => {
    try {
        const user = await user_1.default.findById(req.user.id);
        if (!user) {
            res.status(404).json({ msg: "User not found" });
...
router.post("/:movieId/mark-watched", authMiddleware_1.authenticate, async (req, res) => {
    try {
        const { groupId, watchedAt, watchedDate, watchedLocation, watchedWhere, watchedWith, watchedNotes, } = req.body;
        if (!groupId) {
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(groupIdValue) || !mongoose_1.default.Types.ObjectId.isValid(movieId)) {
        ...
        const group = await Groups_1.default.findById(groupIdValue);
        if (!group) {
        ...
        const hasInvalidWatchedWith = watchedWithIds.some((memberId) => !mongoose_1.default.Types.ObjectId.isValid(memberId) || !groupMemberIds.has(memberId));
        ...
        await user.save();
        (0, socket_1.getIO)().to(groupIdValue).emit("group:movie_added", { movie });
...
router.get("/favorites", authMiddleware_1.authenticate, async (req, res) => {
    try {
        const user = await user_1.default.findById(req.user.id).populate("favorites");
        if (!user) {
            res.status(404).json({ msg: "User not found" });
            return;
        }
...
router.post("/favorites/:movieId", authMiddleware_1.authenticate, async (req, res) => {
    try {
        const user = await user_1.default.findById(req.user.id);
        if (!user) {
            res.status(404).json({ msg: "User not found" });
